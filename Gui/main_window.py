...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.dict_languages = supported_languages
        ...
        if self.size().width() <= 767:
            ...
            self.user_email = self.UserEmail()

            info_layout.addWidget(self.user_name)
            info_layout.addWidget(self.user_email)
            ...
            self.native_lang = self.NativeLang()
            self.target_lang = self.TargetLang()

            lang_layout.addWidget(self.native_lang)
    ...
    def welcome_landing(self):
        WelcomeMessage = QLabel(
            "Welcome to EduVocs, Where Language mestery meets innovation"
        )
    ...
    def NativeLang(self):
        ...
        self.native_language.setEditable(True)
        self.native_language.addItem("Your Native Language")
        self.native_language.setCurrentIndex(0)

        for lang in self.dict_languages.keys():
            self.native_language.addItem(lang)

        completer = QCompleter(self.dict_languages.keys())
        self.native_language.setCompleter(completer)
    ...
    def TargetLang(self):
        ...
        for lang in self.dict_languages.keys():
            self.target_language.addItem(lang.lower())

        completer = QCompleter(self.dict_languages.keys())
        self.target_language.setCompleter(completer)
    ...
    def click_done_button(self):
        ...
        if not match(name_pattern, user_name):
            ...
            storage.save_user_data(
                ...
                self.dict_languages[self.native_language.currentText()],
                self.dict_languages[self.target_language.currentText()],
            )
            next_window = LearningWindow()
            self.close()
            next_window.show()
