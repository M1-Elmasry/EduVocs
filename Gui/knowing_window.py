...
class KnowingWindow(BaseWindow):
    ...
    def open_knowing_window(self, event):
        print("knowing Window currently opened")

    def open_learning_window(self, event):
        from .learning_window import LearningWindow

        print("learning from knowing opened")
        learning_window = LearningWindow(parent=self)
    ...
    def create_header(self):
        header = QHBoxLayout()
        records_count = len(storage.load_records()["knowing"])
    ...
    def create_record_container(self):
        ...
        for record in storage.load_records()["knowing"][::-1]:
